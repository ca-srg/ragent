...
- **PDF (.pdf)**: 各ページが1つのドキュメントになります（`OCR_PROVIDER=bedrock` が必要）
...
# OCR設定（PDF ベクトル化用）
OCR_PROVIDER=bedrock                                    # OCR プロバイダー（"bedrock" のみ対応。未設定時は PDF をスキップ）
OCR_MODEL=anthropic.claude-3-5-sonnet-20241022-v2:0    # OCR 用 Bedrock モデル（デフォルト）
OCR_TIMEOUT=120s                                        # OCR リクエストタイムアウト（デフォルト: 120s）
OCR_MAX_PAGES=100                                       # PDF の最大ページ数（デフォルト: 100）
```
...
- S3 Vectorsへの安全な保存
- 並行処理による高速化
- PDF ページ抽出（OCR 経由、各ページが1ドキュメント、`OCR_PROVIDER=bedrock` が必要）
